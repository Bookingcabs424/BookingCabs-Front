...
class MasterTermsConditionPolicyController {

  static async getTermConditionsDetails(req, res) {
    try {
      ...
      console.error('Error in getTermConditionsDetails:', error);
      res.status(500).json({
        status: 'error',
  ...
  static async addTermConditionsDetails(req, res) {
    try {
      const {
        country_id,
        booking_type,
      ...
      const sql = `INSERT INTO master_terms_condition_policy (${columns.join(', ')}) VALUES (${placeholders})`;
      ...
      console.error('Error in addTermConditionsDetails:', error);
      res.status(500).json({
        status: 'error',
        message: 'Internal server error'
      });
  ...
  static async updateTermConditionsDetails(req, res) {
    try {
      const {
        auto_id,
        country_id,
      ...
      const sql = `UPDATE master_terms_condition_policy SET ${setClause} WHERE id = :autoId`;
      ...
      console.error('Error in updateTermConditionsDetails:', error);
      res.status(500).json({
        status: 'error',
        message: 'Internal server error'
      });
  ...
  static async termConditionsPolicyStatus(req, res) {
    try {
      ...
      const sql = 'UPDATE master_terms_condition_policy SET status = :status, modified_by = :userId, modified_date = :modifiedDate WHERE id IN (:idArray)';
      ...
      console.error('Error in termConditionsPolicyStatus:', error);
      res.status(500).json({
        status: 'error',
        message: 'Internal server error'
      });
...
export default MasterTermsConditionPolicyController;