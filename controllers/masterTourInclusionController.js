...
class MasterTourInclusionController {
  static async getTourInclusionDetails(req, res) {
    try {
      ...
      let sql = `
                ...
                FROM master_tour_inclusion as mti 
      ...
      console.error("Error in getTourInclusionDetails:", error);
      res.status(500).json({
        status: "error",
  ...
  static async addTourInclusionDetails(req, res) {
    try {
      const {
        country_id,
        booking_type,
      ...
      const sql = `INSERT INTO master_tour_inclusion (${columns.join(
        ", "
      )}) VALUES (${placeholders})`;
      ...
      if (result) {
        res.json({
          status: "success",
          data: result,
          message: "Tour inclusion added successfully",
        });
      } else {
        res.status(400).json({
          status: "failed",
      ...
      console.error("Error in addTourInclusionDetails:", error);
      res.status(500).json({
        status: "error",
        message: "Internal server error",
      });
  ...
  static async updateTourInclusionDetails(req, res) {
    try {
      const {
        auto_id,
        country_id,
      ...
      const sql = `UPDATE master_tour_inclusion SET ${setClause} WHERE id = :auto_id`;
      ...
      console.error("Error in updateTourInclusionDetails:", error);
      res.status(500).json({
        status: "error",
        message: "Internal server error",
      });
  ...
  static async tourInclusionStatus(req, res) {
    try {
      ...
      const sql =
        "UPDATE master_tour_inclusion SET status = :status, modified_by = :user_id, modified_date = :modified_date WHERE id IN (:idArray)";
      ...
      res.json({
        status: "success",
        message: "Tour inclusions status updated successfully",
      });
    } catch (error) {
      console.error("Error in tourInclusionStatus:", error);
      res.status(500).json({
        status: "error",
        message: "Internal server error",
      });
  ...
  static async deleteTourInclusions(req, res) {
    try {
      ...
      const sql = "DELETE FROM master_tour_inclusion WHERE id IN (:idArray)";
      ...
      res.json({
        status: "success",
        message: "Tour inclusions deleted successfully",
        affectedRows: result, // result is number of rows deleted
      });
    } catch (error) {
      console.error("Error in deleteTourInclusions:", error);

      if (error.name === "SequelizeForeignKeyConstraintError") {
        return res.status(400).json({
          status: "failed",
          message:
            "Cannot delete tour inclusion as it is being used by other records",
        });
      }
  ...
  // Soft delete tour inclusions (update status to 2)
  static async softDeleteTourInclusions(req, res) {
    try {
      const { id, user_id } = req.body;
      ...
      const sql =
        "UPDATE master_tour_inclusion SET status = 2, modified_by = :user_id, modified_date = :modified_date WHERE id IN (:idArray)";
      ...
      res.json({
        status: "success",
        message: "Tour inclusions soft deleted successfully",
      });
    } catch (error) {
      console.error("Error in softDeleteTourInclusions:", error);
      res.status(500).json({
        status: "error",
        message: "Internal server error",
      });
...
export default MasterTourInclusionController;
